...
本文档说明如何把 `fpa5_modified_files.zip` 覆盖到原 `fpa5` 项目中，并运行分布式实验、收集结果和查看关键输出。

## 1. 准备项目目录

假设你已经有原始 `fpa5.zip` 和修改文件包 `fpa5_modified_files.zip`。

在 PowerShell 中执行：

```powershell
Expand-Archive fpa5.zip -DestinationPath .
cd fpa5
```

然后把修改文件覆盖进去。假设 `fpa5_modified_files.zip` 在当前目录的上一级：

```powershell
Expand-Archive ..\fpa5_modified_files.zip -DestinationPath . -Force
```
...
```text
fl_audit/distributed/http_client.py
