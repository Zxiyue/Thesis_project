...
from fl_audit.distributed.state_store import client_url, ensure_runtime_dirs, entity_logs_dir, load_config, logs_dir, output_dir
...
class KGCService:
    ...
    def setup(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "paillier": keypair_to_dict(self.pkey),
    ...
    def round_init(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for cid in self.U:
            msg = {"r": r, "i": cid, "alpha": alpha, "modelHash": model_hash_r, "m": masks[cid].tolist(), "rho": str(rhos[cid])}
            ...
            send_json("KGC", f"C{cid}", url, "MaskMsg", msg, r, client_id=cid, timeout=timeout, log_path=self.comm_log)
    ...
    def final_confirm(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        xagg = np.array([int(x) for x in payload["xAgg"]], dtype=np.int64)
        ...
        if int((np.abs(x_model - xagg) != 0).sum()) != 0:
            raise RuntimeError("KGC model differential check failed")
        if pedersen.commit(self.ped_params, x_model.tolist(), 0) != com_agg:
            raise RuntimeError("KGC commitment check failed")
