...
from fl_audit.distributed.state_store import client_url, cs_url, ensure_runtime_dirs, entity_logs_dir, kgc_url, load_config, logs_dir, output_dir
...
class CSService:
    def __init__(self, cfg: Dict[str, Any], config_path: str):
        ...
        self.trace: Dict[str, Any] = {"rounds": [], "runtime": {"distributed": True, "client_mode": "individual"}}
    ...
    def _setup_protocol(self) -> None:
        ...
        for cid, base in endpoints.items():
            info = get_json(base + "/public_key", timeout=self._timeout())
        ...
        setup_resp = send_json(
            ...
            "SetupReq",
            {"client_public_keys": self.client_public_keys, "client_endpoints": endpoints},
            0,
        ...
        self.pkey = keypair_from_dict(setup_resp["paillier"])
        ...
        for cid, base in endpoints.items():
            ...
            send_json("CS", f"C{cid}", base + "/init_model", "InitModel", init_payload, 0, client_id=cid, timeout=self._timeout(), log_path=self.comm_log)
    ...
    def start_experiment(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self.trace = {"rounds": [], "runtime": {"distributed": True, "client_mode": "individual"}}
        ...
        for r in range(1, rounds + 1):
            ...
            for cid in active:
                resp = send_json(
                    "CS",
                    f"C{cid}",
                    client_url(self.cfg, cid, "/train_upload"),
                    "TrainUploadReq",
                    {"r": r, "alpha": alpha, "modelHash": self.current_model_hash, "device": device},
                    r,
                    client_id=cid,
                    timeout=self._timeout(),
                    log_path=self.comm_log,
            ...
            verify_resp = send_json("CS", "KGC", kgc_url(self.cfg, "/verify_uploads"), "VerifyUploadsReq", {"r": r}, r, timeout=self._timeout(), log_path=self.comm_log)
            ...
            comp_resp = send_json("CS", "KGC", kgc_url(self.cfg, "/compensation"), "CompensationReq", {"r": r}, r, timeout=self._timeout(), log_path=self.comm_log)
            cdrop = [int(x) for x in comp_resp["Cdrop"]]
            com_drop = int(comp_resp["ComDrop"])
            craw, cagg, com_raw, com_agg = self.cs.aggregate(valid_msgs, cdrop, com_drop, self.pkey.public, self.ped_params)
            ...
            for cid in U_r1:
                send_json("CS", f"C{cid}", client_url(self.cfg, cid, "/receive_agg"), "AggMsg", {"AggMsg": agg_msg}, r, client_id=cid, timeout=self._timeout(), log_path=self.comm_log)
            ...
            final_payload = {
                ...
                **model_payload(self.model),
                "rootUp": root_up,
                "xAgg": [int(x) for x in xagg.tolist()],
                "sigModel": sig_model,
            }
            final_resp = send_json("CS", "KGC", kgc_url(self.cfg, "/final_confirm"), "ModelMsg", final_payload, r, timeout=self._timeout(), log_path=self.comm_log)
            ...
            for cid in U_r1:
                ...
                send_json("CS", f"C{cid}", client_url(self.cfg, cid, "/receive_model"), "ModelBroadcast", broadcast_payload, r, client_id=cid, timeout=self._timeout(), log_path=self.comm_log)
